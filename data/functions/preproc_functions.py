...
def proc_skelet_wudi(gestures, used_joints, gesture, STATE_NO=5, NEUTRUAL_SEG_LENGTH=8):
    ...
    for i in range(STATE_NO):  #HMM states force alignment
        ...
        begin_frame = int(count)
        end_frame = int(count+seg_length)
        Targets_gesture[begin_frame:end_frame, :]= numpy.tile(targets.T,(int(seg_length), 1))
...
def proc_skelet(skelet, _3D=True):
    ...
    for i in range(l): pheight[i] = linalg.norm(pc[:,i]-ph[:,i])
